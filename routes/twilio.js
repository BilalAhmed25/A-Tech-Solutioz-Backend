...
router.post("/voice-handler", bodyParser.urlencoded({ extended: false }), async (req, res) => {
    ...
    try {
        // Log the call with the current platform
        await insertCallLog(To, "No answer", userID, CallSid);

        // 2. Transcription (Common for both, but check provider support)
        ...
        // 4. AMD Setup
        const amdOptions = isTelnyx ? {
            // Telnyx specific TeXML parameters
        ...
        dial.number(amdOptions, To);

        res.type("text/xml").send(response.toString());

    } catch (err) {
        console.error(`${platform} Voice Handler Error:`, err);
...
router.post("/voice-handler-old", bodyParser.urlencoded({ extended: false }), async (req, res) => {
    const { To, userID, CallSid } = req.body;
    const response = new VoiceResponse();
